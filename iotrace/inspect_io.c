...
size_t read(int fd, void *buf, size_t count)
{
  ...
  orig_read  = (orig_open_f_type)dlsym(RTLD_NEXT,"read");
...
size_t fread(void *ptr, size_t size, size_t nmemb, FILE *stream)
{
  ...
  orig_fread  = (orig_open_f_type)dlsym(RTLD_NEXT,"fread");
  iosize=orig_fread(ptr,size,nmemb,stream);
  if (iosize>=0)
    {
      printf("%s fread(%d %d)=%d\n",inspectio_str,fd,count,iosize);      
      tab_fd_to_readcall[fd]++;
