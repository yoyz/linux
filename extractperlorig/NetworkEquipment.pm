# @(#) $Id: NetworkEquipment.pm,v 1.26.4.1 2011/05/24 14:52:58 peyrardj Exp $
...
sub set_name($)
{
    ...
    {
	clmWarning($fct,"name $name doesn't respect name convention RFC 952 : 's/_/-/g'");
	$name=~s/_/-/g;
	$class->{"name"}=$name;
	return $class->{"name"};
    }
    ...
    {
	clmOutput($fct,"Error : name $name doesn't respect name convention RFC 952");
	exit 1;
	return undef;
    }
}
...
sub set_ip()
{
    ...
    {
	clmOutput($fct,"Error : $ip is not an IPv4 address");
	return undef;
    }
}
