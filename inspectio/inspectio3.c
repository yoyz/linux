...
void printListIoByFileChainlist(struct ioByFileChainlist * self,FILE * FD) 
 { 
   ...
   while(iter!=NULL) 
   {
     ...
     fprintf(FD,"fd:%5.1d state:%6.6s wtime :%d.%d rtime :%d.%d filename:%147.147s\n",
	     iter->iobfo->fd,
	     state,
...
void add_write_time(struct ioByFileChainlist * chainlist_elem,  struct timeval tv0,  struct timeval tv1)
{
  ...
  printf("%d.%d      %d.%d         %d.%d       %d.%d\n",
	 ...
	 chainlist_elem->iobfo->tab_fd_to_writetime_seconds,
	 chainlist_elem->iobfo->tab_fd_to_writetime_useconds);
  */
}


void add_read_time(struct ioByFileChainlist * chainlist_elem,  struct timeval tv0,  struct timeval tv1)
{
...
size_t read(int fd, void *buf, size_t count)
{
  ...
  if (size>=0)
      ...
      if (chainlist_elem!=NULL)
	{
	  ...
	  add_read_time(chainlist_elem,tv0,tv1);
	}      
    }
  return size;
}
...
size_t write(int fd, void *buf, size_t count)
{
  ...
  if (size>=0)
      ...
      if (chainlist_elem!=NULL)
	{
	    ...
	    add_write_time(chainlist_elem,tv0,tv1);
	}
    }
...
void init() 
{
  ...
  /* pid=getpid(); */
  /* sprintf(logfile,"/tmp/tmp/log.%d",pid); */
