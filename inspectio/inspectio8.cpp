...
void Iio::dump()
{
  ...
  for (i=0;i<iiof.size();i++)
    {
      write_size+=iiof[i].iac.writesize;      
      read_size+=iiof[i].iac.readsize;

      writetime_useconds+=iiof[i].iac.writetime_useconds;  
      readtime_useconds+=iiof[i].iac.readtime_useconds;
    }
  ...
  if (readtime_useconds+writetime_useconds)
      ...
      for (i=0;i<iiof.size();i++)
	{
      ...
      // print BW=0 
      if (readtime_useconds+writetime_useconds)
	fprintf(FD,"IOTIME[                 %7.1f ] R/W [         %7.1f         ]BW[ %7.1f MB/s ]\n",
		f_readtime_seconds+f_writetime_seconds,
		f_readsize_MB+f_writesize_MB,
